...
import { View, Text, StyleSheet, TouchableOpacity, Dimensions, FlatList, Animated, Alert } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import { useTheme } from '../ThemeContext';

// You would fetch this from the backend in a real app
...
export default function SubscriptionScreen() {
  ...
  const handleCancel = () => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Yes, Cancel', style: 'destructive', onPress: () => {/* call cancel API here */} },
      ]
    );
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}> 
      <Text style={[styles.header, { color: theme.text }]}>Your Subscription</Text>
      <View style={styles.subscriptionCard}>
        <Ionicons name="card-outline" size={36} color={theme.primary} style={{ marginBottom: 12 }} />
        <Text style={[styles.planTitle, { color: theme.text }]}>{userSubscription.plan}</Text>
        <Text style={[styles.status, { color: userSubscription.status === 'Active' ? theme.primary : theme.error }]}>{userSubscription.status}</Text>
        <Text style={[styles.renewal, { color: theme.textSecondary }]}>Renews on: {userSubscription.renewal}</Text>
        <TouchableOpacity style={[styles.cancelButton, { backgroundColor: theme.error }]} onPress={handleCancel}>
          <Text style={{ color: '#fff', fontWeight: 'bold' }}>Cancel Subscription</Text>
        </TouchableOpacity>
      </View>
      <FlatList
        ref={flatListRef}
